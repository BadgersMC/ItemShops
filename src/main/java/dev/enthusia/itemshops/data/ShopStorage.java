...
public final class ShopStorage {
    ...
    public void saveAll(ShopManager mgr) {
        ...
        for (Shop s : mgr.all()) {
            ...
            cfg.set(k+".freeze.enabled", s.isFrozen());
            cfg.set(k+".freeze.until_ms", s.frozenUntilMs());
        }
    ...
    public void loadAll(ShopManager mgr) {
        ...
        for (String key : root.getKeys(false)) {
            ...
            boolean frozen = s.getBoolean("freeze.enabled", false);
            long untilMs = s.getLong("freeze.until_ms", 0L);
            if (frozen) {
